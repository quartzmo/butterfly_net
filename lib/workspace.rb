...
#      end
      ...
      #interpret the butterfly_net pseudocommands
      if /^\s*(oo+ps|bc|m(\s+.+|))\s*$/ === stmts

        return evaluate__orig(ctx,stmts,file,line)
      elsif /^\s*bn(\s+.+|)\s*$/ === stmts
        bn_cmd_result = evaluate__orig(ctx,stmts,file,line)
        @test_case = bn_cmd_result if bn_cmd_result and bn_cmd_result.is_a?(ButterflyNet::TestCase)
      ...
      #  save_copy_of_stdout_while(output=[]) do
          ...
          @test_case.add_command(stmts, result) if @test_case

       # end
      rescue Exception => ee
        @test_case.add_command(stmts, result, ee) if @test_case
